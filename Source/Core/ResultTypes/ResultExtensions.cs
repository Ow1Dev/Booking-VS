...
public static class ResultExtensions
{
    ...
    public static Result<Result> Bind<TIn>(
        this Result<TIn> result,
    ...
    public static TResult Match<RV1, TResult>(this Result<RV1> r, Func<RV1, TResult> successFunc,
        Func<Error[], TResult> errorFunc)
    {
        return r.IsSuccess ? successFunc(r.Value) : errorFunc(r.Errors);
    }
    
    public static async Task<TOut> Match<TIn, TOut>(this Task<Result<TIn>> resultTask, 
        Func<TIn, TOut> successFunc,
        Func<Error[], TOut> errorFunc)
    {
        var result = await resultTask;

        return result.IsSuccess ? successFunc(result.Value) : errorFunc(result.Errors);
    }
}